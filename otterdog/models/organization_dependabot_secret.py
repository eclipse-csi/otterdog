...
class OrganizationDependabotSecret(Secret):
    """
    Represents a Dependabot Secret defined on organization level.
    """
    ...
    def validate(self, context: ValidationContext, parent_object: Any) -> None:
        ...
        if is_set_and_valid(self.visibility):
            ...
            if self.visibility == "private" and org.settings.plan == "free":
                context.add_failure(
                    FailureType.ERROR,
                    f"{self.get_model_header(parent_object)} has 'visibility' '{self.visibility}', "
                    f"which is not available for free plan organizations.",
                )
            elif self.visibility not in {"public", "private", "selected"}:
                context.add_failure(
                    FailureType.ERROR,
                    f"{self.get_model_header(parent_object)} has invalid visibility '{self.visibility}'.",
                )
            ...
            if self.visibility != "selected" and len(self.selected_repositories) > 0:
                context.add_failure(
                    FailureType.WARNING,
                    f"{self.get_model_header(parent_object)} has visibility '{self.visibility}', "
                    f"but selected_repositories is set and will be ignored.",
                )
    ...
    def get_jsonnet_template_function(self, jsonnet_config: JsonnetConfig, extend: bool) -> str | None:
        return f"orgs.{jsonnet_config.create_org_dependabot_secret}"

    @classmethod
    async def apply_live_patch(
        cls,
        patch: LivePatch[OrganizationDependabotSecret],
        org_id: str,
        provider: GitHubProvider,
    ) -> None:
        match patch.patch_type:
            case LivePatchType.ADD:
                await provider.add_org_dependabot_secret(
                    org_id,
                    await unwrap(patch.expected_object).to_provider_data(org_id, provider),
                )

            case LivePatchType.REMOVE:
                await provider.delete_org_dependabot_secret(org_id, unwrap(patch.current_object).name)

            case LivePatchType.CHANGE:
                await provider.update_org_dependabot_secret(
                    org_id,
